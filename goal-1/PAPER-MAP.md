...
| `L1b-EQ` | §1b | With injective `φ` and `ψ`, `φ(x) = ψ(x)` becomes `w = θ(w)` for `θ = ψ ∘ φ⁻¹` | `CodeIso.ofCodes` formalizes the generated-submonoid isomorphism underlying this rewrite, but the diagonal-equation reduction itself is not used or formalized. The empty-word boundary is recorded under `L1c-EMPTY` and `A-035` | source-confirmed |
...
| `L2-COUPLE` | §2 | Forward rules, inverse rules, and halt-to-star switches run forward and then backward | The paper prints the union of forward, starred-inverse, and halt-switch families, but omits conflict/well-formedness assumptions and a proof under correct inverse semantics. `Coupling.turnaround` and `returnGadget` use disjoint phase tags and exact ambient inverse laws; correspondence with the printed family remains open | spec-gap |
...
| `L4a-SIM1` | §4a(1) | Every source step is simulated by a finite reversible macro-run | `History.strictlyReachable_of_source_step` gives the abstract positive simulator step and exact inverse. `HistoryCompiler.historyMachine_step_forward` and `history_forward_reachable` realize it as finite two-tape microsteps. Only correspondence with Lecerf's one-tape marker layout remains open | source-confirmed |
...
| Repaired inverse semantics | `Tape.checkedWrite`, `Tape.moveEquiv`, `Rule.tapeAction`, `Rule.apply_eq_some_iff_undo_eq_some`, `FiniteMachine.step_eq_some_iff_reverseStep_eq_some`, `FiniteMachine.toPEquiv` | 3 (implemented atomically; ordinary one-tape phase-control lowering open) |
| Effective source transition | `Source.universalEvalSearchStep`, `Source.universalEvalSearchStep_halts_iff_eval_dom`, `Source.universalEvalSearchStep_primrec`, `Source.evalSearchStart_joint_primrec` | 3 (implemented replacement source; Stage 6 separately supplies the fixed finite source) |
