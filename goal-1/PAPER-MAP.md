...
| `L1b-EQ` | §1b | With injective `φ` and `ψ`, `φ(x) = ψ(x)` becomes `w = θ(w)` for `θ = ψ ∘ φ⁻¹` | `CodeIso.ofCodes` formalizes the generated-submonoid isomorphism underlying this rewrite, but the diagonal-equation reduction itself is not used or formalized. The empty-word boundary is recorded under `L1c-EMPTY` and `A-035` | cited-background |
