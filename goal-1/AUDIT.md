...
| `A-023` | Semantic phase composition is not yet an ordinary finite phase-control machine | `Rule.tapeAction` composes a checked write and move `PEquiv`; `FiniteMachine.reverseStep` performs move-back/check/restore as one macro-step. No generated `normal`/`move` rule table is exposed | Treat the checked atomic `PEquiv` theorem as the permitted cleaner equivalent result. Do not claim a finite syntactic phase compiler until one is generated and proved correct for a finite alphabet | isolated-obligation |
